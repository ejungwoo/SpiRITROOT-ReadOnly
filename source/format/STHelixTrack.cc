...
void STHelixTrack::RemoveHitCluster(STHitCluster *cluster)
{
  auto numHits = fHitClusters.size();
  for (auto iCluster = 0; iCluster < numHits; iCluster++) {
    if (fHitClusters[iCluster] == cluster) {
      fHitClusters.erase(fHitClusters.begin()+iCluster);
      break;
    }
  }
}
